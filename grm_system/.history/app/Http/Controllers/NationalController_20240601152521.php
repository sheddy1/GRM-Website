...
class NationalController extends Controller
{
    ...
    function national_reports(Request $request){
        ...
        // gieviance summary pie chart

        // report page code for charts

        // report date session
        $report_year = session::get('report_summary_resolved');

        // report month session
        $report_month = session::get('report_summary_resolved');
        ...
        //end of grieviance summart pie chart
        ...
        return view('national.reports', [
            ...
            'cat8'=>$cat8,
            'month' => $month,
            'report_percentage_resolved' => $report_percentage_resolved,
            'report_percentage_new' => $report_percentage_new,
            'report_percentage_review' => $report_percentage_review
        ]);
    }
    ...
    {
        $report_summary_year= $_COOKIE['report_summary_year'];

        session::put('report_summary_resolved', $report_summary_year);

        return back();

    }

    // report summary get month code
    function report_summary_month()
    {
        $report_summary_month= $_COOKIE['report_summary_month'];

        session::put('report_summary_resolved', $report_summary_month);
    ...
    function test()
    {
        $a = session::get('report_percentage_review') ;
        echo $a;
    }
}
