...
class NationalController extends Controller
{
    ...
    public function list_download(Request $request)
    {
        $data = $request->info_share;
         ...
         if($data == "excel"){
            return Excel::download(new listExport, 'Grieviance List.xlsx');
         }
         else{
            return Excel::download(new listExport, 'Grieviance list.csv');
         }
    }
    ...
    public function category_list(Request $request){
        ...
        //session::put('cat_state', $state);

        //$a = session::get('cat_state');
        //echo $a;
    ...
    // report summary get year code
    function report_summary_year()
    {
        $report_summary_month= $_COOKIE['report_summary_year'];

        echo $report_summary_month;

        //getting count for resolved variable
        $report_summary_resolved_session = grieviance::whereYear('created_at', '=', $report_year)
        ->whereMonth('created_at', '=', $report_month)
        ->where('resolved', '=', 'yes')
        ->get()->count();

        //session for summary resolved
        session::put('report_summary_resolved', $report_summary_resolved_session);
        $report_summary_resolved = session::get('report_summary_resolved');

        //getting count for review variable
        $report_summary_review_session = grieviance::whereYear('created_at', '=', $report_year)
        ->whereMonth('created_at', '=', $report_month)
        ->where('resolved', '=', 'no')
        ->get()->count();

        //session for summary review
        session::put('report_summary_review', $report_summary_review_session);
        $report_summary_review = session::get('report_summary_review');

        //getting count for new variable
        $report_summary_new_session = grieviance::whereYear('created_at', '=', $report_year)
        ->whereMonth('created_at', '=', $report_month)
        ->where('attended', '=', 0)
        ->get()->count();

        //session for summary new
        session::put('report_summary_new', $report_summary_new_session);
        $report_summary_new = session::get('report_summary_new');
    ...
    function test()
    {
        ...
        $report_month = date('F', strtotime($report_date));
